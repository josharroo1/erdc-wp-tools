...
function cac_restrict_registration_page() {
    if (is_page('cac-registration') && is_user_logged_in() && !current_user_can('manage_options')) {
        wp_redirect(home_url('/account/')); // Redirect to the account page or any other appropriate page
        exit;
    }
}
